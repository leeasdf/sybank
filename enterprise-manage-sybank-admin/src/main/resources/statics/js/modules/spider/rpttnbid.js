...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.rptTnBid.id == null ? "spider/rpttnbid/save" : "spider/rpttnbid/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.rptTnBid),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "spider/rpttnbid/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "spider/rpttnbid/info/"+id, function(r){
                vm.rptTnBid = r.rptTnBid;
            });
		},
