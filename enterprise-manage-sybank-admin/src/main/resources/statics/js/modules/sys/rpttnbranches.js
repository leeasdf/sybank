$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/rpttnbranches/list',
        datatype: "json",
        colModel: [			
			...
			{ label: '被投资法定代表?人', name: 'legalPerson', index: 'legal_person', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.rptTnBranches.id == null ? "sys/rpttnbranches/save" : "sys/rpttnbranches/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.rptTnBranches),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "sys/rpttnbranches/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/rpttnbranches/info/"+id, function(r){
                vm.rptTnBranches = r.rptTnBranches;
            });
		},
