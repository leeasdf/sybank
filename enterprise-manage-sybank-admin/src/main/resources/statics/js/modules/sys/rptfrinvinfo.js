$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/rptfrinvinfo/list',
        datatype: "json",
        colModel: [			
			...
			{ label: '认缴出资币种', name: 'currency', index: 'currency', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.rptFrinvInfo.id == null ? "sys/rptfrinvinfo/save" : "sys/rptfrinvinfo/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.rptFrinvInfo),
			    success: function(r){
			    	if(r.code === 0){
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function(){
				$.ajax({
					type: "POST",
				    url: baseURL + "sys/rptfrinvinfo/delete",
                    contentType: "application/json",
		...
		getInfo: function(id){
			$.get(baseURL + "sys/rptfrinvinfo/info/"+id, function(r){
                vm.rptFrinvInfo = r.rptFrinvInfo;
            });
		},
